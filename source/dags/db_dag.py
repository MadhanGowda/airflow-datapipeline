...
from source.dag_helper.sample_helper.db_helper import (  # noqa isort:skip
...
cleanup_data = PostgresOperator(
    ...
    # Truncate the tables before loading of data.
    sql=["TRUNCATE table test.DCM_TEST_DATA1;",
         "TRUNCATE table test.metadata;",
         "TRUNCATE table test.splitdata"
         ],
    autocommit=True,
...
insert_raw_data = PostgresOperator(
    ...
    postgres_conn_id='airflow_postgres',
    dag=datapipeline_dag,
    # load the raw data to DB from csv file.
...
insert_meta_data = PostgresOperator(
    ...
    postgres_conn_id='airflow_postgres',
    dag=datapipeline_dag,
    # load the meta data to DB from csv file.
    sql='''COPY test.metadata FROM
    '/var/lib/postgresql/data/pgdata/metadata.csv'
    DELIMITER ',' CSV HEADER;''',
    autocommit=True,
    database='sample_airflow'
)
...
load_to_database = PostgresOperator(
    ...
    postgres_conn_id='airflow_postgres',
    dag=datapipeline_dag,
    # load the new generated data to DB from csv file.
